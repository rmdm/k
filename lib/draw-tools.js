...
export function drawGrid (ctx, offsetW, offsetH, width, height, nlines,
    ...
    getColor, getYColor, fromRight, lineWidth, textSize, nightGrade, zoomGrade) {
...
export function drawLinesLegend (ctx, x, lines, lineLabels, lineStates, lineGrades, lineColors, lineWidth, rullerWidth,
    position, closestIndex, offsetW, offsetH, topGraphOffsetH, bottomGraphOffsetH, width, height, left, right, range, currentMaxHeight, currentMinHeight, xDiff,
    textSize, textFont, getRullerColor, getLegendColor, getLegendBorderColor, getLegendFontColor, getBackgroundColor,
    ...
    let lowestTopOffset = topGraphOffsetH + height
    ...
    for (const line of lines) {
        ...
        if (lineStates[label] || lineGrades[label] > 0) {
            ...
            const topOffset = height - (approxValue - currentMinHeight) * scaleY
