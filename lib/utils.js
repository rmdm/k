...
export function drawLinesLegend (ctx, x, lineKinds, lineLabels, lineStates, lineGrades, lineColors, lineWidth,
    ...
    alphaGrade, nightGrade, coordinates, zoomGrade, format, drawArrow, drawAll) {
    ...
    if (drawAll) {
        ...
        ctx.font = 'bold ' + legendFont
        ctx.fillStyle = color
        const valWidth = ctx.measureText(sum).width
        ctx.fillText(sum, coordinates.x + legendPadding + contentWidth - valWidth, textOffsetH)
    }
